...
When packaging the final submission:

- Do not include `node_modules/`.
- Do not include `dist/` unless specifically requested.
- Do not include `coverage/` if it makes the zip large.
- Do not include `*.sqlite` runtime database files.
- Include `package-lock.json`.
- Include all source files and tests.

The existing `.gitignore` excludes generated/runtime files:
